  ...
  <q-page class="flex flex-center">
    <q-form @submit.prevent="onSubmit" class="rounded q-my-lg flex column">
      ...
      <q-select
        ...
        label="Status"
        :rules="[(val) => (val && val.length > 0) || 'Stat is required']"
      />
      <q-select
        outlined
        class="full-width q-pb-lg"
      ...
      <q-select
        ...
        :options="optionsReviewers"
        label="Reviewers"
        :rules="[(val) => (val && val.length > 0) || 'Reviewer is required']"
      />
      <q-input
        outlined
...
export default defineComponent({
  ...
  methods: {
    ...
    async getReviewer() {
      const reviewers = await axios.get("http://localhost:3000/reviewers");
      this.optionsReviewers = reviewers.data.map((item) => {
        return item.name;
      });
    },
    onSubmit() {
